...
public class SendQueueReportTests
{
    ...
    private static readonly Partner Alpha = new() { Id = 1, Name = "Alpha", As2Id = "ALPHA" };
    private static readonly Partner Beta = new() { Id = 2, Name = "beta", As2Id = "BETA" };
    private static readonly Partner Gamma = new() { Id = 3, Name = "Gamma", As2Id = "GAMMA" };
    ...
    public void AgesAreWholeMinutesAndTotalsAddUp()
    {
        ...
        Assert.Equal(new SendQueuePartnerReport("Alpha", "ALPHA", 3, 90, 1, 2, 300, "connection refused", Now.AddMinutes(-2)), alpha);

        var beta = report.Partners[1];
        Assert.Equal(10, beta.OldestWaitingMinutes);
        Assert.Equal(0, beta.OldestAwaitingMdnMinutes);
    }

    [Fact]
    public void MessagesOfAPartnerThatNoLongerExistsCountInTheTotals()
    {
