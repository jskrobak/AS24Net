...
namespace AS24Net.Services.Api;
...
{
    /// <summary>
    /// <c>message.sent</c>, <c>message.delivered</c>, <c>message.not_delivered</c>, <c>message.failed</c>,
    /// <c>message.received</c>, <c>certificate.applied</c> or <c>health.changed</c>.
    /// </summary>
    public required string Event { get; init; }
    public DateTimeOffset Timestamp { get; init; } = DateTimeOffset.Now;
    public int? OutgoingMessageId { get; init; }
    ...
    public string? PartnerAs2Id { get; init; }
    public string? IdentityAs2Id { get; init; }
...
/// <summary>
/// Calls webhooks registered through the REST API (per message or per token) in the background, with retries.
/// The request body is JSON; when a secret is configured, the header <c>X-AS24Net-Signature</c> carries its
...
{
    public const string SignatureHeader = "X-AS24Net-Signature";
    public const string EventHeader = "X-AS24Net-Event";
    ...
    private void Record(TransferEventType type, TransferEventLevel level, WebhookPayload payload, string message, string? details)
    {
        transferEvents.Record(new TransferEvent
        {
            ...
            OutgoingMessageId = payload.OutgoingMessageId,
            ReceivedMessageId = payload.ReceivedMessageId,
        });
    }
}
