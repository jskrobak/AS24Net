    ...
    ITimeService timeService) : IHealthCheck
{
    ...
    public static HealthCheckResult Evaluate(bool running, bool paused, DateTime? started, DateTime? lastRun,
        int activeTransfers, int sendIntervalSeconds, DateTime now)
    {
        ...
        if (!running)
            return HealthCheckResult.Unhealthy("The send service is not running, no messages are sent.", data: data);

        if (paused)
            return HealthCheckResult.Degraded("The send service is paused, no messages are sent until it is resumed.", data: data);
        ...
        return HealthCheckResult.Healthy($"The send service is running, {activeTransfers} transfer(s) active.", data);
    }
}
