...
public sealed record RetentionCutoffs(DateTime Content, DateTime InformationEvents, DateTime Events, DateTime FinishedMessages)
{
    /// <summary>Messages received again within this time are recognised as duplicates by their record.</summary>
    public const int MinFinishedMessagesDays = 30;
    ...
    public static RetentionCutoffs From(GlobalSettings settings, DateTime now)
    {
        ...
        var finished = Math.Max(settings.DeleteFinishedMessagesAfterDays, MinFinishedMessagesDays);
...
public sealed record RetentionResult(DateTime Started, DateTime Finished, int EventsArchived, int OutboxFilesDeleted,
    int EventsDeleted, int OutgoingMessagesDeleted, int ReceivedMessagesDeleted, string? Error)
{
    public override string ToString() =>
        $"{EventsArchived} transfer log record(s) archived, {EventsDeleted} deleted, {OutboxFilesDeleted} outbox file(s) deleted, " +
        $"{OutgoingMessagesDeleted} sent and {ReceivedMessagesDeleted} received message record(s) archived and deleted";
}
...
/// archive in full and their details and hook parameters are removed (a hook run cannot be repeated then); the
/// payloads of messages delivered before that time are deleted from the outbox.</item>
...
/// <item>Sent and received messages that need nothing more are written to the archive and deleted after
/// <see cref="GlobalSettings.DeleteFinishedMessagesAfterDays"/>.</item>
/// </list>
/// Nothing unfinished is touched: messages waiting, failed or waiting for their MDN, in either direction. The
/// received payloads themselves are left where they are, they belong to the integration. Every
    ...
    ILogger<RetentionService> logger) : BackgroundService
{
    ...
    public const string OutgoingKind = "outgoing-messages";
    public const string ReceivedKind = "received-messages";
    ...
    public async Task<RetentionResult> RunAsync(CancellationToken cancellationToken = default)
    {
        ...
        int archived = 0, outbox = 0, deleted = 0, outgoing = 0, received = 0;
        ...
        try
        {
            ...
            outgoing = await StepAsync("deleting sent messages", () => DeleteOutgoingMessagesAsync(cutoffs.FinishedMessages, archive, cancellationToken), errors);
            received = await StepAsync("deleting received messages", () => DeleteReceivedMessagesAsync(cutoffs.FinishedMessages, archive, cancellationToken), errors);
        }
        ...
        {
            result = new RetentionResult(started, timeService.GetCurrentTime(), archived, outbox, deleted, outgoing,
                received, errors.Count == 0 ? null : string.Join(" ", errors));
            LastResult = result;
    ...
    /// <summary>
    /// Deletes the files in the outbox that belong only to messages delivered before the cutoff. Files of no message
    /// are left alone: the directory may hold files the application did not put there.
    /// </summary>
    private async Task<int> DeleteOutboxFilesAsync(GlobalSettings settings, DateTime deliveredBefore, CancellationToken cancellationToken)
    {
        ...
        using var scope = serviceScopeFactory.CreateScope();
        var sendQueue = scope.ServiceProvider.GetRequiredService<IOutgoingMessageRepository>();
        var count = 0;
    ...
    private async Task<int> DeleteOutgoingMessagesAsync(DateTime createdBefore, string archive, CancellationToken cancellationToken)
    {
        using var scope = serviceScopeFactory.CreateScope();
        var messages = scope.ServiceProvider.GetRequiredService<IOutgoingMessageRepository>();
        var count = 0;
        while (!cancellationToken.IsCancellationRequested)
        {
            var batch = await messages.GetFinishedAsync(createdBefore, BatchSize, cancellationToken);
            if (batch.Count == 0)
                break;

            await RetentionArchive.AppendAsync(archive, OutgoingKind, batch.Select(ArchivedOutgoingMessage.From), m => m.Created, cancellationToken);
            count += await messages.DeleteAsync(batch.Select(m => m.Id).ToList(), cancellationToken);
        }

        return count;
    }

    private async Task<int> DeleteReceivedMessagesAsync(DateTime createdBefore, string archive, CancellationToken cancellationToken)
    {
        using var scope = serviceScopeFactory.CreateScope();
        var messages = scope.ServiceProvider.GetRequiredService<IReceivedMessageRepository>();
        var count = 0;
        while (!cancellationToken.IsCancellationRequested)
        {
            var batch = await messages.GetFinishedAsync(createdBefore, BatchSize, cancellationToken);
            ...
            await RetentionArchive.AppendAsync(archive, ReceivedKind, batch.Select(ArchivedReceivedMessage.From), m => m.Created, cancellationToken);
            count += await messages.DeleteAsync(batch.Select(m => m.Id).ToList(), cancellationToken);
        }
