...
- Contacts per connection, shown next to a failed connection test
...
| *Contacts* | people to call when the connection does not work, with e-mails and phones; listed with the partners by the REST API |
...
| `GET /api/v1/partners`, `/identities` | names usable when sending; partners with the contacts of their connections |
...
| `GET /api/v1/connections` | the connections with their settings, certificates, contacts and the AS2 names of their partners |
