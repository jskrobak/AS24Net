...
namespace AS24Net.Services.TransferEvents;
...
/// <summary>
/// Stores <see cref="TransferEvent"/> records in batches in the background (so recording never slows down an AS2
/// transfer) and once a day marks records older than <see cref="GlobalSettings.ArchiveEventsAfterDays"/> as archived.
/// </summary>
    ...
    ILogger<TransferEventLog> logger) : BackgroundService, ITransferEventLog
{
    ...
    {
        transferEvent.Timestamp = timeService.GetCurrentTime();
        ...
        transferEvent.MessageId = Truncate(transferEvent.MessageId, 250);
        transferEvent.FileName = Truncate(transferEvent.FileName, 250);
